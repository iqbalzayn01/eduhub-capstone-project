...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchRandomEvents = async () => {
      try {
        ...
        // Ambil 3 acara acak dari daftar acara
        const randomIndexes = getRandomIndexes(events.length, 3);
        const randomEventsData = randomIndexes.map((index) => events[index]);
  ...
  // Fungsi untuk mendapatkan indeks acak
        ...
        <div className="lg:mx-[4rem] sm:max-sm mx-[1rem] py-[1rem]">
          ...
          <section id="hero">
            ...
            <div className="flex flex-col lg:flex-row gap-5 mt-4 lg:mt-8">
              {randomEvents.slice(0, 1).map((event) => (
              ...
              <div>
                <div className="flex flex-col gap-5 lg:w-[35rem] h-full">
                  {randomEvents.slice(1, 3).map((event) => (
                    <article
                      key={event.id}
                      className={`p-4 lg:p-12 flex-1 bg-black bg-center bg-cover rounded-2xl overflow-hidden text-white`}
                    >
